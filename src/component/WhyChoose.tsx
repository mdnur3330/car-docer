...
export default function WhyChoose() {
  ...
  return (
    <div className="my-20">
      <Container>
        {/* Heading */}
        <div className="space-y-4 text-center my-10 px-4">
          <h3 className="text-[#ff3811] font-semibold text-lg">Core Features</h3>
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold">Why Choose Us</h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-6 sm:gap-8">
          {item.map((i) => (
            <div
              key={i.id}
              className="border flex flex-col items-center justify-center gap-3 border-gray-300 rounded-lg p-4 text-center font-medium hover:shadow-md transition"
            >
              <Image
                src={i.img}
                width={40}
                ...
                className="mx-auto"
              />
              <span className="text-sm sm:text-base">{i.titel}</span>
            </div>
          ))}
        </div>
      </Container>
